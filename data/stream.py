...
class InstanceStream(Stream):
    ...
    def __init__(self, dataset: Dataset, order=None, frac=1.0, shuffle=False, cls_names: list = None,
                 init_frac: float = 0.0):
...
class ClassStream(Stream):
    ...
    def __init__(self, train_dataset: Dataset, test_dataset: Dataset = None, class_size: int = 1,
                 class_frac: float = 1.0,
                 class_batch_seq: list = None, test_frac: float = 0.2, max_cls_num=-1, cls_names: list = None,
                 init_data: Dict = None):
        super().__init__(cls_names)
        ...
        self.init_data = (init_class_concept_mapping, Subset(train_dataset,
                                                             init_indices if cf == 1.0 else random.sample(init_indices,
                                                                                                          int(cf * len(
                                                                                                              init_indices)))))

        self.train_data = [(class_idx, Subset(train_dataset,
                                              indices if cf == 1.0 else random.sample(indices, int(cf * len(indices)))),
                            class_concept_mapping)
        ...
        self.test_data = [(class_idx, Subset(test_dataset,
                                             indices if cf == 1.0 else random.sample(indices, int(cf * len(indices)))),
                           class_concept_mapping)
    ...
    def create_class_batches(dataset, class_size, class_batch_seq, max_cls_num):
        ...
        if not class_batch_seq:
            ...
            for i, subclasses in enumerate(class_batch_seq):
                class_batch_seq[i] = (
                i, subclasses, {c: i for c in subclasses})  # todo: use task/superclass switch c: c vs c: i, when offline learning (1 batch)

        indices_per_class_batch = [
            (class_idx, CollectionUtils.flatten_list([indices_per_class[cls] for cls in classes]), concept_mapping)
