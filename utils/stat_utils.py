...
class MathUtils:
    ...
    @staticmethod
    def bhat_dist(m1, cv1, m2, cv2):
        cvm = (cv1 + cv2) / 2.0
        delta = m1 - m2
        m = torch.dot(delta, torch.matmul(torch.inverse(cvm), delta))
        l = 0.5 * torch.log(torch.det(cvm) / (torch.sqrt(torch.det(cv1) * torch.det(cv2))))
        return m + l

    @staticmethod
    def bhat_coeff(bhat_dist):
        return torch.exp(-bhat_dist)

    @staticmethod
    def mah_dist(x, m, inv_cov):
        delta = m - x
        m = torch.dot(delta, torch.matmul(inv_cov, delta))
        return torch.sqrt(m)

    @staticmethod
    def mah_coeff(mah_dist, self_dists):
        return torch.softmax(torch.stack(mah_dist, self_dists), dim=-1)[0]
