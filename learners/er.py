...
from core.clearn import ContinualLearner, LossTracker, TimeTracker
...
class SubspaceBuffer(ReplayBuffer):
    def __init__(self, max_centroids: int, max_instances: int, mode: str, centroids_frac: float=1.0):
    ...
    def sample(self, x_batch, y_batch, weights, **kwargs):
        if self.mode == 'centroid_sample':
            return self.centroid_sample(x_batch)
        elif self.mode == 'class_sample':
            return self.class_sample(x_batch)
        else:
            raise ValueError('Mode: {0} is not supported!'.format(self.mode))

    def centroid_sample(self, x_batch):
    ...
    def class_sample(self, x_batch):
        num_samples = len(self.buffers) * len(x_batch)
        ...
        i = 0
        class_buffers = {y: [sb for sb in b if len(sb) > 0] for y, b in self.buffers.items()}

        for _ in range(len(x_batch)):
            for y_buffer in class_buffers.values():
                sb = random.randint(0, len(y_buffer) - 1)
                (rx, ry, rw) = random.choice(y_buffer[sb])
                sampled_x_batch[i, :] = rx[:]
                sampled_y_batch[i] = ry
                sampled_weights[i] = rw
                i += 1
...
class StreamingExperienceReplay(ContinualLearner):

    def __init__(self, model: NeuralNet, replay_buffer: ReplayBuffer, mbs: int=64):
        super().__init__()
        self.model = model
    ...
    def update(self, x_batch, y_batch, **kwargs):
        ...
        ext_data = TensorDataset(ext_x_batch, ext_y_batch)
        for xb, yb in DataLoader(ext_data, batch_size=self.mbs, shuffle=True):
            if len(xb) <= 1:
                continue
...
class ClassIncrementalExperienceReplay(ContinualLearner, LossTracker, TimeTracker):

    def __init__(self, model: nn.Module, replay_buffer_size: int, optimizer, scheduler, batch_size: int,
    ...
    def predict_prob(self, x_batch):
        with torch.no_grad():
            y = self.model(x_batch.to(self.device))
    ...
    def update(self, x_class_batch, y_class_batch, **kwargs):
        ...
        if new_cls not in self.replay_buffer and self.replay_buffer_size > 0:
            self.replay_buffer[new_cls] = du.rand_selection(x_class_batch, self.replay_buffer_size, 'cpu').to(
                self.replay_buffer_device)

        losses = []
        self.__set_model_train()

        for xb, yb, ib in DataLoader(class_batch, batch_size=self.batch_size, shuffle=True):
            if len(xb) <= 1:
                continue

            self.optimizer.zero_grad()

            loss = self.__er_update(xb, yb)
            losses.append(loss)
    ...
    def __er_update(self, xb, yb):
        total_loss = 0.0

        logits = self.model(xb.to(self.device))
        loss = self.loss_function(logits, yb.long().to(self.device))
        loss.backward()
        total_loss += loss.item()

        replay_buffer = {k: v for k, v in self.replay_buffer.items() if k != yb[0]}

        if len(replay_buffer) > 0:
            s = self.batch_size

            for y, rxb in replay_buffer.items():
                logits = self.model(du.rand_selection(rxb, s, self.device))
                y_target = torch.ones(len(logits)).to(self.device).long() * y

                loss = self.loss_function(logits, y_target)
                loss.backward()
                total_loss += loss.item()
